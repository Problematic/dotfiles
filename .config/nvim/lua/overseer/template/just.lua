-- Shadows overseer.nvim's builtin just template. Identical except that a
-- parameter whose default is an expression rather than a literal (e.g.
-- `jobs=jobs`) is left optional instead of being spliced into argv as
-- `variable jobs`. Delete once upstream handles non-string defaults.
  ...
  for _, recipe in pairs(recipes) do
    if not recipe.private then
      ...
      for _, param in ipairs(recipe.parameters) do
        -- `just --dump` reports a non-literal default (a variable, a call, an
        -- expression) as a table, not a string. Only a string is a usable
        -- default; otherwise leave the parameter empty so the builder omits it
        -- and just applies the default itself.
        local default = type(param.default) == "string" and param.default or nil
